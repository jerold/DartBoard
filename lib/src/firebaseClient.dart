...
class FirebaseClient {
  ...
  Future<Note> createNote(
      String boardUid, String sessionUid, String ownerUid, String text) async {
  ...
  Future<Item> createItem(
    ...
    List<String> options,
      ...
      await _createItem(
          boardUid, sessionUid, ownerUid, categoryUid, text, options);
  ...
  Future<Category> createCategory(
    ...
    String color,
      ...
      await _createCategory(boardUid, sessionUid, title, description, color);

  /// [createSession] creates a session
  Future<Session> createSession(String boardUid, int targetTime) async {
    final session = await _createSession(boardUid, targetTime: targetTime);
  ...
  Future<Board> createBoard(
    ...
    final board = await _createBoard(
      ownerUid,
      title,
      description,
    );
  ...
  Future<User> userFromFirebaseUser(firebase.User fbUser) async {
    ...
    final userSnapshot = userRef.snapshot.val();
    if (userSnapshot == null) {
      return await _createUser(fbUser.uid, fbUser.displayName);
    }
    return (serializers
            .deserializeWith<User>(User.serializer, userSnapshot)
            .toBuilder()
              ..uid = fbUser.uid)
        .build();
  }
  ...
  Future editItemText(String text, Item item) async {
    await _refs
        .item(item.boardUid, item.sessionUid, item.uid)
        .child('text')
        .set(text);
  }
  ...
  Future hideCategory(Category category) async {
    await _refs
        .category(category.boardUid, category.sessionUid, category.uid)
        .child('visible')
        .set(false);
  }
  ...
  Future hideNote(Note note) async {
    await _refs
        .note(note.boardUid, note.sessionUid, note.uid)
        .child('visible')
        .set(false);
  }
  ...
  Future setSessionTarget(Session session, int targetTime) async {
    await _refs
        .session(session.boardUid, session.uid)
        .child('targetTime')
        .set(targetTime);
  }
  ...
  Future present(Item item, int startTime) async {
    ...
    await _refs
        .session(item.boardUid, item.sessionUid)
        .child('presentedUid')
        .set(item.uid);
    await _refs
        .session(item.boardUid, item.sessionUid)
  ...
  Future<Category> _createCategory(
    ...
    final newCategoryRef =
        await _refs.categories(boardUid, sessionUid).push().future;
    ...
    await newCategoryRef
        .set(serializers.serializeWith(Category.serializer, category));
    return category;
  }
  ...
  Future<Session> _createSession(
    ...
    await newSessionRef
        .set(serializers.serializeWith(Session.serializer, session));
    return session;
  }
  ...
  Future<Board> _createBoard(
      String ownerUid, String title, String description) async {
